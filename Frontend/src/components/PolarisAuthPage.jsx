...
import { ArrowLeft, Mail, Lock, Eye, EyeOff, User, ArrowRight, Building2, Briefcase, ShieldCheck } from "lucide-react";

/**
 * POLARIS Dedicated Authentication & Sign In Page
 * Location: Frontend/src/components/PolarisAuthPage.jsx
 * Route: #login | #auth | #register
 */
export default function PolarisAuthPage({ onSuccess, onNavigateHome, initialPrompt, currentUser, onLogout }) {
  ...
  return (
    <div className="min-h-[82vh] w-full flex flex-col items-center justify-center py-6 sm:py-10 px-4 relative animate-fade-in">
      {/* Top Navigation Bar: Back to Portal Overview */}
      <div className="w-full max-w-[560px] mb-4 flex items-center justify-between">
        <button
          type="button"
          onClick={onNavigateHome}
          className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-cyan-400 hover:text-cyan-300 border border-slate-700/80 text-xs font-semibold transition-all cursor-pointer shadow-sm"
        >
          <ArrowLeft className="w-4 h-4" />
          <span>Return to Portal Overview</span>
        </button>
        <div className="text-[11px] font-mono text-slate-500 uppercase tracking-wider">
          POLARIS Identity Gate
        </div>
      </div>

      {currentUser ? (
        /* Authenticated User Status Card */
        <div className="w-full max-w-[560px] bg-slate-900 border border-slate-700/80 rounded-2xl p-6 sm:p-8 shadow-2xl text-center space-y-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-white">Currently Authenticated</h2>
            <p className="text-sm text-slate-300 mt-1">
              Signed in as <span className="font-semibold text-cyan-400">{currentUser.name || currentUser.email}</span>
            </p>
        ...
        /* Dedicated Sign In / Register Card */
        <div
          className="w-full max-w-[560px] bg-slate-900 border border-slate-700/80 rounded-2xl px-6 sm:px-8 py-6 sm:py-7 shadow-2xl shadow-cyan-950/60 relative overflow-hidden"
        ...
        >
          ...
          {/* Header Tag & Titles */}
          <div className="mb-4">
            <div className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-cyan-950/90 border border-cyan-500/40 text-[10px] font-bold tracking-widest text-cyan-400 uppercase mb-2">
            ...
            <p className="text-xs text-slate-400 mt-1">
              {initialPrompt ||
                (mode === "login"
                  ? "Enter your credentials to access verified datasets & scientific records."
                  : "Register to access polar telemetry, publish datasets, and contribute models.")}
            </p>
          </div>

          {/* Segmented Tab Switch */}
          <div className="flex p-1 bg-slate-950/90 border border-slate-800 rounded-xl mb-4">
            <button
              type="button"
              onClick={() => {
                setMode("login");
          ...
          {/* Error Alert Box */}
          {errorMsg && (
            <div className="mb-3.5 p-2.5 rounded-xl bg-rose-950/70 border border-rose-500/60 text-rose-200 text-xs flex items-center gap-2">
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            {mode === "login" ? (
              ...
              <>
                {/* Name & Email 2-column in register mode */}

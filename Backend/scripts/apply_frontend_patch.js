...
console.log('Target file:', assetFile);
let content = fs.readFileSync(assetFile, 'utf8');

// Load RBAC components
const { RBAC_COMPONENTS } = require('./prepare_frontend_patch.js');

// 1. Inject RBAC_COMPONENTS right before function KC()
if (!content.includes('function PolarisAuthGateModal')) {
  const kcIndex = content.indexOf('function KC(){');
  if (kcIndex === -1) {
    throw new Error('Could not find function KC(){ in file');
  }
  content = content.slice(0, kcIndex) + '\n' + RBAC_COMPONENTS + '\n\n' + content.slice(kcIndex);
  console.log('✓ Injected RBAC components before KC()');
} else {
  console.log('PolarisAuthGateModal already present');
}

// 2. Modify q1 signature to accept auth props
// Old: function q1({activeTab:t,setActiveTab:e,onOpenSearch:n,theme:th,setTheme:setTh,lang:curLang="EN",setLang:setLProp}){
// New: function q1({activeTab:t,setActiveTab:e,onOpenSearch:n,theme:th,setTheme:setTh,lang:curLang="EN",setLang:setLProp,currentUser:cuUser,onOpenAuthModal:onAuth,onLogout:onLogOut}){
...
if (content.includes(oldQ1Sig)) {
  content = content.replace(oldQ1Sig, newQ1Sig);
  console.log('✓ Updated q1 function signature with auth props');
} else if (content.includes(newQ1Sig)) {
  console.log('q1 signature already has auth props');
} else {
  console.warn('⚠️ Could not match oldQ1Sig');
}

// 3. Modify primaryNav in q1 to add "Admin Review" tab when cuUser?.role === "admin"
...
if (content.includes(oldPrimaryNav)) {
  content = content.replace(oldPrimaryNav, newPrimaryNav);
  console.log('✓ Updated primaryNav to include Admin Review tab for admin role');
} else {
  console.log('primaryNav already updated or not matched');
}

// 4. Update Desktop controls in q1 to render User Badge / Logout or Sign In button
// Look for Search & Toggle Controls in q1
...
if (content.includes(oldDesktopControls)) {
  content = content.replace(oldDesktopControls, newDesktopControls);
  console.log('✓ Updated Desktop controls with Auth badge & Logout button');
} else {
  console.log('Desktop controls already updated or not matched');
}

// 5. Update Mobile Drawer in q1 to also have Auth Controls
...
if (content.includes(oldMobileControls)) {
  content = content.replace(oldMobileControls, newMobileControls);
  console.log('✓ Updated Mobile navbar with User initial/Sign In button');
}

// 6. Update ow (Datasets page)
// Old signature: function ow({onNavigateToVisualizer:t}){
// New signature: function ow({onNavigateToVisualizer:t,currentUser:cuUser,onTriggerDownload:triggerDl,onOpenUploadModal:openUpload}){
...
if (content.includes(oldOwSig)) {
  content = content.replace(oldOwSig, newOwSig);
  console.log('✓ Updated ow signature with RBAC props');
} else {
  console.log('ow signature already updated or not matched');
}

// 7. In ow header, add "+ Upload New Dataset" button for researcher or admin
...
if (content.includes(oldOwHeaderActions)) {
  content = content.replace(oldOwHeaderActions, newOwHeaderActions);
  console.log('✓ Added "+ Upload New Dataset" button in ow header for researcher/admin');
} else {
  console.log('ow header actions already updated or not matched');
}

// 8. In ow card download button, direct download anchor with telemetry
const oldCardDownloadBtn = `                  /* Primary 1-Click Action: Protected Telemetry Download */
                  u.jsxs("button", {
                    type: "button",
                    onClick: () => {
...
const newCardDownloadBtn = `                  /* Primary 1-Click Action: Direct Browser File Download */
                  u.jsxs("a", {
                    ...
                    onClick: () => {
                      try {
                        const tok = localStorage.getItem("polaris_token");
                        if (tok) {
                          fetch(\`/api/datasets/download/\${encodeURIComponent(card.id || card.fileName || "dataset")}?token=\${encodeURIComponent(tok)}&redirect=false\`).catch(()=>{});
                        }
                      } catch(e){}
                    },
                    title: \`Download \${card.name} (\${card.isSingleFile ? "Raw File" : "ZIP Bundle"})\`,
...
if (content.includes(oldCardDownloadBtn)) {
  content = content.replace(oldCardDownloadBtn, newCardDownloadBtn);
  console.log('✓ Kept direct browser download anchor on card');
}

// 9. Update KC state and handlers
const oldKcStart = `function KC(){
...
const handleLoginSuccess = (user, token) => {
  ...
  showToast(\`Welcome back, \${user.name || user.email}! Authenticated as \${user.role.toUpperCase()}.\`);
...
if (content.includes(oldKcStart)) {
  content = content.replace(oldKcStart, newKcStart);
  console.log('✓ Updated KC component with RBAC state and download handlers');
} else {
  console.log('KC state already updated or not matched');
}

// 10. Update KC return JSX to pass props to q1, ow, replace GC with PolarisAdminReviewConsole, and add Modals & Toast
...
if (content.includes(oldKcReturn)) {
  content = content.replace(oldKcReturn, newKcReturn);
  console.log('✓ Updated KC return JSX with modals, toast, and RBAC components');
} else {
  console.log('KC return JSX already updated or not matched');
}

// Write back to assets/index-v2.js and dist/assets/index-v2.js
...
console.log('All frontend patches applied successfully!');
